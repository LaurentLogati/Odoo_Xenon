...
class XenonSaleOrder(models.Model):
    ...
    #    _logger.info('logLLO_compute_purchase_order_count1 : ' + str(order.purchase_order_count))

    @api.depends('order_line.purchase_line_ids.order_id')
    def _compute_purchase_order_count(self):
        """Compute number of purchase orders linked to this sale order.
        Works with both direct sale_line_id links and via the 'origin' fallback.
        """
        for order in self:
            # 1️ Cas normal : relation directe via purchase_line_ids
            purchase_orders = order.order_line.mapped('purchase_line_ids.order_id')
            ...
            _logger.info(
                "logLLO_compute_purchase_order_count1 (%s) : %s (via %s)",
                order.name,
                order.purchase_order_count,
                "direct" if purchase_orders else "origin"
            )
...
class XenonSaleOrderLine(models.Model):
    ...
    @api.depends('purchase_line_ids')
    def _compute_purchase_count(self):
        """Compute how many purchase order lines are linked to each sale order line."""
        database_data = self.env['purchase.order.line'].sudo()._read_group(
            [('sale_line_id', 'in', self.ids)],
            ['sale_line_id'],
            ['__count']
        )
        # database_data = [(sale_line_record, count), ...]
        mapped_data = {sale_line.id: count for sale_line, count in database_data}
    ...
    def create(self, values):
        lines = super(XenonSaleOrderLine, self).create(values)
        # Do not generate purchase when expense SO line since the product is already delivered
        lines.filtered(
            lambda line: line.state == 'wait' and not line.is_expense
    ...
    def write(self, values):
        ...
        result = super(XenonSaleOrderLine, self).write(values)
